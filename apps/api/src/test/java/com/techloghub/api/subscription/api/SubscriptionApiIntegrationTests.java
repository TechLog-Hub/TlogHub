...
class SubscriptionApiIntegrationTests {
	...
	void requestsVerifiesManagesAndUnsubscribesCompanySubscriptions() throws Exception {
		...
		mockMvc.perform(get("/api/v1/subscriptions/manage/{manageToken}", manageToken))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.maskedEmail").value("de***@example.com"))
			.andExpect(jsonPath("$.status").value("active"))
			.andExpect(jsonPath("$.companies.length()").value(2));

		mockMvc.perform(post("/api/v1/subscriptions/manage/{manageToken}/companies", manageToken)
				.contentType(MediaType.APPLICATION_JSON)
		...
		mockMvc.perform(delete(
				"/api/v1/subscriptions/manage/{manageToken}/companies/{companySlug}",
				manageToken,
				"kakao-subscription"
			))
			.andExpect(status().isOk())
		...
		mockMvc.perform(post("/api/v1/subscriptions/manage/{manageToken}/unsubscribe", manageToken))
			.andExpect(status().isOk())
